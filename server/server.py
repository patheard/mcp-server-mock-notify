...
# Configuration
DEFAULT_BASE_URL = os.getenv("MOCK_NOTIFY_BASE_URL", "https://notify.ai-jam.cdssandbox.xyz")
...
async def signup_user(
    email: str, password: str, base_url: str = DEFAULT_BASE_URL
) -> str:
    """
    ...
    if result.success:
        ...
        return json.dumps(
            {"error": result.error, "status_code": result.status_code}, indent=2
        )


@mcp.tool()
async def create_service(
    name: str, api_key: str, description: str = "", base_url: str = DEFAULT_BASE_URL
) -> str:
    """
    ...
    result = await make_request(
        "POST", "/api/services", api_key=api_key, data=data, base_url=base_url
    )
    ...
    if result.success:
        ...
        return json.dumps(
            {"error": result.error or result.data, "status_code": result.status_code},
            indent=2,
        )
...
async def get_services(api_key: str, base_url: str = DEFAULT_BASE_URL) -> str:
    ...
    """
    result = await make_request(
        "GET", "/api/services", api_key=api_key, base_url=base_url
    )
...
async def create_template(
    ...
    """
    data = {"name": name, "subject": subject, "body": body, "service_id": service_id}
    result = await make_request(
        "POST", "/api/templates", api_key=api_key, data=data, base_url=base_url
    )
...
async def get_templates(api_key: str, base_url: str = DEFAULT_BASE_URL) -> str:
    ...
    """
    result = await make_request(
        "GET", "/api/templates", api_key=api_key, base_url=base_url
    )
...
async def send_email_notification(
    ...
    """
    data = {"template_id": template_id, "recipient_email": recipient_email}
    result = await make_request(
        "POST",
...
async def send_sms_notification(
    ...
    """
    data = {"template_id": template_id, "recipient_phone": recipient_phone}
    result = await make_request(
        "POST", "/api/notifications/sms", api_key=api_key, data=data, base_url=base_url
    )
...
async def get_message_status(
    message_id: str, api_key: str, base_url: str = DEFAULT_BASE_URL
) -> str:
    """
    ...
    """
    result = await make_request(
        "GET", f"/api/messages/{message_id}/status", api_key=api_key, base_url=base_url
    )
...
async def get_messages(api_key: str, base_url: str = DEFAULT_BASE_URL) -> str:
    ...
    """
    result = await make_request(
        "GET", "/api/messages", api_key=api_key, base_url=base_url
    )
